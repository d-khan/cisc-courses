...
!!! info "Learning Outcomes"
    ...
    * Explain the memory hierarchy and why it improves performance.
    * Describe RAM, ROM, cache memory, and auxiliary storage.
    * Explain temporal and spatial locality.
    * Calculate hit ratio and miss ratio.
    * Compare SRAM and DRAM technologies.
    * Explain how cache memory works.
    * Describe direct-mapped cache organization.
    * Explain cache hits, misses, tags, and valid bits.
    * Compare write-through and write-back cache policies.
    * Describe ROM technologies including PROM, EPROM, EEPROM, and Flash.
    * Explain how memory technologies affect computer performance and cost.
...
C. To replace RAM completely  
D. To store operating systems permanently

<details>
<summary>Show Answer</summary>

Answer: B. To reduce memory access time

Cache memory stores frequently accessed data closer to the CPU, reducing average memory access time.
...
### Question 2
Which type of locality is demonstrated by repeatedly accessing the same variable inside a loop?
...
Answer: C. Temporal locality

Temporal locality states that recently accessed data is likely to be accessed again soon.
...
### Question 3
A cache recorded 900 hits and 100 misses. What is the hit ratio?
...
C. SRAM is faster but more expensive than DRAM.  
D. DRAM is used primarily for CPU cache.

<details>
<summary>Show Answer</summary>

Answer: C. SRAM is faster but more expensive than DRAM.

SRAM uses more transistors per bit and is commonly used for cache memory.
...
### Question 5
What is the purpose of the valid bit in a cache entry?

A. Stores part of the memory address  
B. Indicates whether the cache block contains valid data  
C. Stores parity information  
D. Counts cache hits

<details>
<summary>Show Answer</summary>

Answer: B. Indicates whether the cache block contains valid data.

A cache entry cannot be considered a hit unless its valid bit is set.
...
### Question 6
A direct-mapped cache contains 16 blocks. Which cache block will memory address 45 map to?
...
Cache Index = Address mod Cache Size

45 mod 16 = 13

Therefore, memory address 45 maps to cache block 13.
...
### Question 7
A cache has a hit rate of 95%. Which statement is true?

A. Every memory access requires main memory access.  
B. 95% of memory accesses are satisfied by the cache.  
C. The cache is always full.  
D. The miss penalty is zero.

<details>
<summary>Show Answer</summary>

Answer: B. 95% of memory accesses are satisfied by the cache.

Only 5% of accesses require fetching data from lower levels of the memory hierarchy.
...
### Question 8
Why can increasing cache block size eventually reduce performance?

A. Larger blocks always reduce hit rate.  
B. Larger blocks increase CPU clock frequency.  
C. Larger blocks increase miss penalty and reduce the number of blocks that fit in the cache.  
D. Larger blocks eliminate spatial locality.

<details>
<summary>Show Answer</summary>

Answer: C. Larger blocks increase miss penalty and reduce the number of blocks that fit in the cache.

Very large blocks can increase transfer time and create more cache conflicts.
...
### Question 9
A processor uses a write-through cache policy. What happens during a write operation?

A. Data is written only to cache.  
B. Data is written only to main memory.  
C. Data is written to both cache and main memory.  
D. Data is discarded after being written.

<details>
<summary>Show Answer</summary>

Answer: C. Data is written to both cache and main memory.

Write-through keeps cache and memory consistent at all times.
...
### Question 10
A program sequentially processes every element of a large array. Which type of locality is primarily being exploited?

A. Temporal locality only  
B. Spatial locality only  
C. Both temporal and spatial locality equally  
D. No locality

<details>
<summary>Show Answer</summary>

Answer: B. Spatial locality only

Adjacent array elements are stored in nearby memory locations, making spatial locality the dominant factor.
