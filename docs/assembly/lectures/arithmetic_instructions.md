...
!!! info "Learning Outcomes"
    ...
    - Use arithmetic instructions such as **INC, DEC, ADD, SUB, MUL, IMUL, DIV, and IDIV** in x86 assembly programs.
    - Perform arithmetic operations using registers, memory locations, and immediate values.
    - Explain how multiplication and division results are stored in processor registers.
    - Interpret the effects of arithmetic instructions on registers and processor flags.
    - Trace and debug arithmetic operations using **gdb**.
    - Develop simple assembly programs that solve mathematical expressions using arithmetic instructions.
...
The following code solves the equation $var4 = (var1 + var2) * var3$, where $var1$, $var2$ and $var3$ are initialized variables, whereas $var4$ is an un-initialized variable. The output will be saved in $var4$ with a value of 21.
...
__When the divisor is 1 byte -__ The dividend is assumed to be in the AX register (16 bits). After division, the quotient goes to the AL register, and the remainder goes to the AH register.
...
## __Concept Check Questions__

### Question 1
What is the value stored in EAX after the following instructions execute?
...
### Question 3
Which registers contain the quotient and remainder after the following division?
...
### Question 4
What is the value of EAX after executing the following instructions?
...
### Question 5
What is the purpose of the .bss section in an assembly language program?

<details>
<summary>Show Answer</summary>

The .bss section is used to reserve storage for uninitialized variables.

Example:

assembly result resb 1

Answer: It stores uninitialized variables.
...
### Question 6
Determine the final value stored in var4.
...
#### Question 7
After executing the following code, what values are stored in AL and AH?
...
#### Question 8
Why does the following instruction produce a 64-bit result when multiplying two 32-bit values?
...
For a 32-bit multiplication:

- Multiplicand is stored in EAX.
- Multiplier is stored in EBX (or another register/memory location).
- The product may require up to 64 bits.
...
Answer: The product can exceed 32 bits, so the result is stored in the EDX:EAX register pair.
...
The x86 architecture does not allow memory-to-memory arithmetic operations.
...
Answer: Memory-to-memory arithmetic operations are not allowed.
...
#### Question 10
A program performs the following operation:
...
What is the final quotient stored in EAX?
