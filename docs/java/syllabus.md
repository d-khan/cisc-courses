# **Syllabus (CISC-190 Java Programming)**

## **Welcome to the course!**

Welcome to the course! I look forward to working with you this semester. This course is designed to encourage curiosity,
problem-solving, and hands-on learning in a supportive and respectful environment. Do not be afraid to make
mistakes—they are an important part of learning Computer Science.

## **Course description**

This course provides an introduction to programming using Java. Students will learn the fundamentals of programming and
object-oriented programming while developing general-purpose applications and interactive programs. The course is
designed for students majoring in Computer and Information Sciences, as well as anyone interested in learning Java
programming.

## **Student learning outcome**

--8<-- "docs/shared/slos.md"

## **Course information**

| Course Information | Details                                                         |
|--------------------|-----------------------------------------------------------------|
| Course name        | CISC-190 Java Programming                                       |
| Year               | Fall 2026                                                       |
| Class ID           | 43345                                                           |
| Section            | 3001                                                            |
| Location           | Online                                                          |
| Duration           | Aug 24 – Dec 21 (16 weeks)                                      |
| Professor          | Dr Danish Khan                                                  |
| Contact            | Room M-107P, Office phone: 619-388-7152, Email: dkhan@sdccd.edu |

## **Important dates/deadline**

--8<-- "docs/shared/dates_deadlines_Fall.md"

## **Communication guidelines**

--8<-- "docs/shared/communication.md"

## **Course topics**

| Topics                             | Hands-on activities                             |
|------------------------------------|-------------------------------------------------|
| Introduction to Java Programming   | Getting Started with Java                       |
| Java Programming using Methods     | Working with Methods                            |
| Decision statements and loops      | Using Decision Statements and Loops             |
| Single and Multidimensional Arrays | Working with Single and Multidimensional Arrays |
| Polymorphism                       | Applying Polymorphism                           |
| Exceptions                         | Handling Exceptions                             |
| File operations                    | Working with File Operations                    |
| GUI widgets and interactions       | Building Interactive GUIs                       |
| Multithreading                     | Working with Multithreading                     |

**Detailed Course Roadmap:** The semester-specific schedule, including the order of topics, lecture materials, hands-on
activities, quizzes, and due dates, is available on the [course website](). Students are responsible for following the
Course website throughout the semester.

## **Textbook**

I recommend following my notes and resources provided on the course site. The following resources are for your reference
only.

Liang, Y. Daniel. Introduction to Java Programming and Data Structures, 13th Edition, Pearson, 2024.

## **Software and hardware resources**

- A regular laptop or desktop computer with a reliable internet connection, microphone, and webcam is sufficient for
  this course.
- SD Miramar students may check out laptops from the library, dependent on availability
  at <https://sdmiramar.edu/library/materials-checkout>

## **Grading scheme**

--8<-- "docs/shared/grading_scheme.md"

## **Course evaluation schemes**

| Evaluation type        | Out of 100 |
|:-----------------------|:-----------|
| Weekly lab assignments | 20         |
| Quizzes                | 20         |
| Project-1              | 30         |
| Project-2              | 30         |

## **Official Course Outline**

The official SDCCD course outline can be found
at [Official SDCCD Course Outline](https://sdccd.curriqunet.com/DynamicReports/AllFieldsReportByEntity/16683?entityType=Course&reportId=493)

> **Please note that the textbooks listed in the official course outline may differ from the materials used in this
course section. Refer to this syllabus for the most current course materials and requirements.**

## **Late/due work**

--8<-- "docs/shared/late_work.md"

## **Attendance/Absences**

--8<-- "docs/shared/attendance.md"

## **Use of Artificial Intelligence Tools**

--8<-- "docs/shared/use_of_AI.md"

## **Cheating/Plagiarism**

--8<-- "docs/shared/cheating.md"

## **Honest Academic Conduct**

--8<-- "docs/shared/honest-academic.md"

## **Accommodating Students with Disabilities**

--8<-- "docs/shared/dsps.md"

## **Undocumented & Immigrant Student Services**

--8<-- "docs/shared/undocumented-immigration.md"

## **Career & Life Design Services**

--8<-- "docs/shared/career.md"

## **Rising Scholars**

--8<-- "docs/shared/rising_scholars.md"

## **Sustainability**

--8<-- "docs/shared/sustainability.md"


